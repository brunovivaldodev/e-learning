...
export default function Categoria({ categoria, color, icon, nCursos }: props) {
  ...
  function renderIcon() {
    switch (icon) {
      ...
      case "ChartBarHorizontal":
        return (
          <ChartBarHorizontal size={27} weight={"duotone"} color={color} />
        );
      case "BugDroid":
  ...
  return (
    <div className="flex w-1/4">
      <div
        className={`flex p-2 pr-2 mb-3 items-center ${backgroundColorClass}`}
      >
        <div className="p-2 m-2 bg-white">{renderIcon()}</div>
